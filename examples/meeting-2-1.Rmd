...
```{r clean-names-snake}
lakes_test <- clean_names(lakes, case = "snake")
```
...
lakes_test <- clean_names(lakes, case = "lower_camel")
```
...
```{r}
lakes_large %>%
  mutate(vlarge = ifelse(size >= 1000, "over 1000 acre",
                         "under 1000 acres"))
```
...
```{r}
lakes_large %>%
  mutate(mercury = ifelse(str_detect(pollutant,
                                     pattern = "Mercury"), 
                          TRUE, FALSE))
```
